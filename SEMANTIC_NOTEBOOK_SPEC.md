...
- **Layout classes** — applied additively. `sem-lab` applies page-chrome classes (`pt-12 box-border`, plus `snap-start snap-always` where scroll-snap is enabled) as static attributes; grid/sizing classes come from `sembook:cssClass` in the notebook definition and are merged at runtime via `_applyLayoutClass()`. No `scroll-mt-12`: `pt-12` (included in each lab's border-box height) alone reserves the fixed nav's clearance, and adding `scroll-margin-top` on top of it double-compensated, making every scroll-snap gesture fall 3rem short of the natural `i*100vh` boundary (iteration 12 fix).
